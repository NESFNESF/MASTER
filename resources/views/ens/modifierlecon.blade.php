...
<div class="content_wrapper">
    ...
    <form method="POST" action="{{ route('storeleconmodif') }}" enctype="multipart/form-data">
        @csrf
        <input type="number" name="idU" value="{{ Auth::user()->id }}" hidden>
        <input type="number" name="idcours" value="{{ $cours->id }}" hidden>
      <h4>Choisissez une classe :</h4>
      <br>
        <p><div class="input-group">
      @php
          $class = DB::table('classes')->where('id',$cours->idC)->get()[0];

      @endphp


 <label >{{ $class->nom }} </label>
 <input type="radio" name="idC" value="{{ $class->id }}" checked >


                  </div>
        </p><br><br>
    <input class="form-control" placeholder="Titre de la leçon" name="titre" type="text" value="{{ $cours->titre }}" required>
    <input class="form-control" placeholder="Unité ou Matière" name="unite" type="text" value="{{ $cours->unite }}" required>

    <textarea class="form-control" rows="5" placeholder="Indicateur de compétence"  required name="indicateur">{{ $cours->indicateur }}</textarea>
    <textarea class="form-control" rows="5" placeholder="Situation problème" required name="situation">{{ $cours->situation }}</textarea>
    <textarea class="form-control" rows="10" required placeholder="Rédiger le contenu de votre leçon ici..." name="contenu">{{ $cours->contenu }}</textarea>
